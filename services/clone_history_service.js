...
const clone_data_model = require('../models/clone_data_model')
...
async function addHistoryMinute(currencyFrom, currencyTo, limit) {
  try {
    let response = await clone_data_model.history_minute(currencyFrom, currencyTo, limit)
        ...
        for (let i = listTime.length - 1; i >= 0; i--) {
          ...
          history.save((err) => {
            if (err) {
              ...
              History.collection.findOneAndUpdate(filter, update, {new: true})
                .catch((err) => {
                  console.log(err)
                })
            }
...
async function addHistoryHour(currencyFrom, currencyTo, limit) {
  try {
    let response = await clone_data_model.history_hour(currencyFrom, currencyTo, limit)
    if (response[1] != null && response[1] != []) {
      let listTime = JSON.parse(response[1]).Data.Data
      if (listTime.length != undefined && listTime.length != 0) {
        for (let i = listTime.length - 1; i >= 0; i--) {
...
async function deleteLastHistoryMinute() {
  try {
...
async function deleteLastHistoryHour() {
  try {
...
async function jobAddHistoryMinute() {
  try {
...
async function jobAddHistoryHour() {
  try {
    let listCoupleCurrency = await CoupleCurrency.find({}, {_id: 0})
    listCoupleCurrency.forEach(element => {
...
module.exports = {
  ...
  jobAddHistoryHour,
  jobAddHistoryMinute,
  deleteLastHistoryMinute,
  deleteLastHistoryHour,
  refreshAllHistory,
}
